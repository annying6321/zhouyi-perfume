...
${colorSchemes.map(s => `- ${s.name}（${s.mood}）`).join('\n')}
  ...
  香气关键词：3个词组，每个为「形容词+名词」，中间用·符号连接。例如「清冽的晨风·温润的琥珀·苍劲的雪松」。直接写「香气关键词：」后跟上三个词组。
  
  香气隐喻：以一个具体的嗅觉场景开头，如「如同……」「仿佛……」等。用通感将卦象核心意境转化为可感知的气味体验，这个体验要贴近生活，真实可感。只写感官世界，不直接解释易经。结尾留有余韵。约100-120字。直接写「香气隐喻：」后跟上内容。
  
  运势解析：语气温柔而有力量。先分析处境再给建议。不要用《周易》的原文，语言通俗易懂，多用「此时宜……」「不妨……」「若……则……」。约80-100字。直接写「运势解析：」后跟上内容。
  
  核心原料：3-4种真实制香原料，每种一行。格式为「原料名（一句话解释）」。直接写「核心原料：」后跟上原料列表。
  
  香水推荐：2款真实存在的知名品牌香水。每款格式为「品牌：真实品牌名」「香水名：真实香水名」「前调：」「中调：」「后调：」，各项单独一行。直接写「香水推荐：」后跟上香水信息。
  ...
  - 香水推荐必须真实存在
  - 除了配色标签用【】包裹外，不得使用【】包裹其他内容
  - 每个区块必须以上述指定的标题开头（如「香气关键词：」「香气隐喻：」「运势解析：」「核心原料：」「香水推荐：」）`;
