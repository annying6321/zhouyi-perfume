...
请严格依据以上爻辞经典原文的含义，按以下顺序输出内容。注意：输出内容时必须做到以下两点：
1. 绝对不要在输出中包含任何标题、标签或说明性文字（如「香气关键词」「运势解析」「核心原料」「香水推荐」等）
2. 直接输出内容本身，不要加「针对你的问题」「关于你的问题」等元叙述开头

输出顺序（每一块内容之间用两个换行分隔）：

第一块：3个词组，每个为「形容词+名词」，中间用·符号连接。例如「清冽的晨风·温润的琥珀·苍劲的雪松」。注意不要输出「香气关键词：」这个标题，直接输出这三个词组。

第二块：以一个具体的嗅觉场景开头，如「如同……」「仿佛……」等。用通感将卦象核心意境转化为可感知的气味体验，这个体验要贴近生活，真实可感。只写感官世界，不直接解释易经。结尾留有余韵。约100-120字。不要输出任何标题，直接写内容。

第三块：运势解析。语气温柔而有力量。先分析处境再给建议。不要用《周易》的原文，语言通俗易懂，多用「此时宜……」「不妨……」「若……则……」。约80-100字。注意：绝对不要以「运势解析：」「核心启示是」「具体行动：」「针对你的问题」等任何元叙述开头，直接写一段连贯的、完整的分析文字。

第四块：3-4种真实制香原料，每种一行。格式为「原料名（一句话解释）」。不要输出「核心原料：」标题。

第五块：2款真实存在的知名品牌香水。每款格式为「品牌：真实品牌名」「香水名：真实香水名」「前调：」「中调：」「后调：」，各项单独一行。不要输出「香水推荐：」标题。

输出完毕后，在末尾单独一行写上：【配色：最匹配的配色方案名称】
...
- 香水推荐必须是真实存在的品牌和香水
- 整个输出中，除了配色标签用【】包裹外，不得使用【】包裹其他内容
- 不允许出现「香气关键词：」「运势解析：」「核心原料：」「香水推荐：」等任何标题性文字`;
